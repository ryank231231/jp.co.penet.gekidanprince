  ...
  private static final class MultipartRequestBody extends RequestBody {
    ...
    private long writeOrCountBytes(BufferedSink param1BufferedSink, boolean param1Boolean) throws IOException {
      ...
      for (byte b = 0; b < i; b++) {
        ...
        buffer1.write(MultipartBuilder.DASHDASH);
        buffer1.write(this.boundary);
        buffer1.write(MultipartBuilder.CRLF);
        if (headers != null) {
          int j = headers.size();
          for (byte b1 = 0; b1 < j; b1++)
            buffer1.writeUtf8(headers.name(b1)).write(MultipartBuilder.COLONSPACE).writeUtf8(headers.value(b1)).write(MultipartBuilder.CRLF); 
        } 
        MediaType mediaType = requestBody.contentType();
        if (mediaType != null)
          buffer1.writeUtf8("Content-Type: ").writeUtf8(mediaType.toString()).write(MultipartBuilder.CRLF); 
        long l = requestBody.contentLength();
        if (l != -1L) {
          buffer1.writeUtf8("Content-Length: ").writeDecimalLong(l).write(MultipartBuilder.CRLF);
        ...
        if (param1Boolean) {
          ...
          ((RequestBody)this.partBodies.get(b)).writeTo((BufferedSink)buffer1);
        } 
        buffer1.write(MultipartBuilder.CRLF);
      } 
      buffer1.write(MultipartBuilder.DASHDASH);
...
/* Location:              Y:\classes2-dex2jar.jar!\com\squareup\okhttp\MultipartBuilder.class
