...
public final class ObservableSequenceEqual<T> extends Observable<Boolean> {
  ...
  public void subscribeActual(Observer<? super Boolean> paramObserver) {
    EqualCoordinator<T> equalCoordinator = new EqualCoordinator<T>(paramObserver, this.bufferSize, this.first, this.second, this.comparer);
    paramObserver.onSubscribe(equalCoordinator);
  ...
  static final class EqualCoordinator<T> extends AtomicInteger implements Disposable {
    ...
    EqualCoordinator(Observer<? super Boolean> param1Observer, int param1Int, ObservableSource<? extends T> param1ObservableSource1, ObservableSource<? extends T> param1ObservableSource2, BiPredicate<? super T, ? super T> param1BiPredicate) {
      this.actual = param1Observer;
      this.first = param1ObservableSource1;
      this.second = param1ObservableSource2;
      this.comparer = param1BiPredicate;
      ObservableSequenceEqual.EqualObserver[] arrayOfEqualObserver = new ObservableSequenceEqual.EqualObserver[2];
      this.observers = (ObservableSequenceEqual.EqualObserver<T>[])arrayOfEqualObserver;
      arrayOfEqualObserver[0] = new ObservableSequenceEqual.EqualObserver(this, 0, param1Int);
      arrayOfEqualObserver[1] = new ObservableSequenceEqual.EqualObserver(this, 1, param1Int);
    ...
    public void dispose() {
      if (!this.cancelled) {
        ...
        if (getAndIncrement() == 0) {
          ObservableSequenceEqual.EqualObserver<T>[] arrayOfEqualObserver = this.observers;
    ...
    void drain() {
      if (getAndIncrement() != 0)
        return; 
      ObservableSequenceEqual.EqualObserver<T>[] arrayOfEqualObserver = this.observers;
      ObservableSequenceEqual.EqualObserver<T> equalObserver1 = arrayOfEqualObserver[0];
      SpscLinkedArrayQueue<T> spscLinkedArrayQueue2 = equalObserver1.queue;
      ObservableSequenceEqual.EqualObserver<T> equalObserver2 = arrayOfEqualObserver[1];
  ...
  static final class EqualObserver<T> implements Observer<T> {
    ...
    EqualObserver(ObservableSequenceEqual.EqualCoordinator<T> param1EqualCoordinator, int param1Int1, int param1Int2) {
...
/* Location:              Y:\classes2-dex2jar.jar!\io\reactivex\internal\operators\observable\ObservableSequenceEqual.class
